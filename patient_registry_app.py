...
def normalize_uploaded_files(*field_names: str):
    ...
    for field_name in field_names:
        for item in request.files.getlist(field_name):
            if item and item.filename and item.filename.strip():
                files.append(item)
    return files


def save_uploaded_files(patient_id: int, uploaded_files, category: str = '', title: str = '') -> int:
...
def delete_uploaded_file(patient_file):
    ...
    if os.path.exists(file_path):
        try:
            ...
            print(f'Local file delete error: {exc}')
...
<div class="card">
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <div class="card" style="padding:12px; margin-top:12px;">
      <h3>İlkin görüntü / fayl</h3>
      <p class="muted">Yeni xəstə əlavə ediləndə bir neçə şəkil, PDF, DICOM və ya MRT qovluğu da yükləyə bilərsən.</p>
      ...
      <div class="field">
        ...
        <input type="file" name="initial_folder_files" accept=".dcm,.dicom" multiple webkitdirectory directory>
      </div>
    </div>
...
<div class="card">
  ...
  <form method="post" action="{{ url_for('upload_patient_file', patient_id=patient.id) }}" enctype="multipart/form-data">
    ...
    <div class="field">
      <label>MRT qovluğu seç</label>
      <input type="file" name="folder_files" accept=".dcm,.dicom" multiple webkitdirectory directory>
    </div>
